...
def main():
    ...
    with st.sidebar:
        ...
        mouth_th = st.slider("Soglia Parlato (MAR)", 0.10, 0.40, 0.20, 0.01)
    ...
    with tab1:
        ...
        with col_rt:
            ...
            m1, m2, m3, m4, m5, m6, m7 = st.empty(), st.empty(), st.empty(), st.empty(), st.empty(), st.empty(), st.empty()

        if st.session_state.recording:
            cap = cv2.VideoCapture(int(cam_id))
            tracker = EyeTracker(blink_threshold=blink_th, mouth_threshold=mouth_th)
            start_t = time.time()
            while st.session_state.recording:
                ret, frame = cap.read()
                if not ret: break
                img, data = tracker.process_frame(frame)
                elapsed = time.time() - start_t
                data['relative_time'] = round(elapsed, 3)
                st.session_state.data_list.append(data)

                v_place.image(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
                s_bar.markdown(f"**REC** | ⏱️ {elapsed:.1f}s | 🖼️ {len(st.session_state.data_list)} frames")
                ...
                # STATO VOCE RICHIESTO
                voice_status = "🟢 Sta parlando" if data['is_talking'] else "🔴 In silenzio"
                m4.metric("Stato Voce", voice_status)
                
                cur_metrics = calculate_metrics(st.session_state.data_list)
                m5.metric("Stress Score", f"{cur_metrics['stress_score']}/100")
                m6.metric("Posizione (X, Y)", f"({data['gaze_x']}, {data['gaze_y']})")
                m7.metric("Saccade", f"{data['saccade_distance']:.4f}")
                time.sleep(0.01)
            cap.release()
    ...
    with tab2:
        if st.session_state.data_list:
            ...
            with c4:
                ...
                st.metric("% Parlato", f"{metrics['pct_talking']}%")
            ...
            with g2:
                st.plotly_chart(px.density_heatmap(df, x='gaze_x', y='gaze_y', nbinsx=15, nbinsy=15, color_continuous_scale='Hot'), use_container_width=True)

            fig_tl = make_subplots(rows=2, cols=1, shared_xaxes=True, subplot_titles=('Sguardo', 'EAR'))
            fig_tl.add_trace(go.Scatter(x=df['relative_time'], y=df['gaze_x'], name='X'), row=1, col=1)
            fig_tl.add_trace(go.Scatter(x=df['relative_time'], y=df['gaze_y'], name='Y'), row=1, col=1)
            fig_tl.add_trace(go.Scatter(x=df['relative_time'], y=df['avg_ear'], name='EAR'), row=2, col=1)
            st.plotly_chart(fig_tl, use_container_width=True)
        else:
            st.info("Registra dati per vedere le analisi.")
