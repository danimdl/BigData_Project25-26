...
def main():
    st.set_page_config(page_title="Eye Tracker Pro", layout="wide")
    ...
    with tab1:
        col_video, col_realtime = st.columns([2, 1])
        with col_video:
        ...
        if st.session_state.recording:
            ...
            while st.session_state.recording:
                ...
                m6.metric("Saccade Distance", f"{data['saccade_distance']:.4f}")
                time.sleep(0.01)
            cap.release()
    ...
    with tab2:
        if st.session_state.data_list:
            ...
            with g1:
                ...
                fig_dir = px.bar(dir_df, x='Direzione', y='Percentuale', color='Percentuale', color_continuous_scale='RdYlGn_r')
    ...
    with tab3:
        if st.session_state.data_list:
            df = pd.DataFrame(st.session_state.data_list)
            st.download_button("Scarica CSV", df.to_csv(index=False), "data.csv", "text/csv")
            st.json(calculate_metrics(st.session_state.data_list))
